...
public class DashboardController implements Initializable {
    ...
    @FXML private Label todayApptCountLabel;
    @FXML private BarChart<String, Number> appointmentsChart;
    @FXML private TableView<RendezVous> upcomingAppointmentsTable;
    @FXML private TableColumn<RendezVous, String> colUpcomingTime;
    @FXML private TableColumn<RendezVous, String> colUpcomingPatient;
    @FXML private TableColumn<RendezVous, String> colUpcomingDoctor;
    @FXML private TableColumn<RendezVous, String> colUpcomingMotif;
    @FXML private Button quickPatientBtn;
    @FXML private Button quickAppointmentBtn;
    @FXML private Button quickInvoiceBtn;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        System.out.println("========== DashboardController.initialize() END ==========\n");
    }
    ...
    private void setupRoleBasedUI() {
        try {
            ...
            boolean canCreateAppointment = role == Role.ADMIN || role == Role.SEC;
    ...
    private void setupUpcomingAppointmentsTable() {
        ...
        if (colUpcomingTime != null) {
            colUpcomingTime.setCellValueFactory(new javafx.util.Callback<javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String>, javafx.beans.value.ObservableValue<String>>() {
                @Override
                public javafx.beans.value.ObservableValue<String> call(javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String> param) {
                    RendezVous rdv = param.getValue();
                    if (rdv == null || rdv.getDateHeureDebut() == null) {
                        return new javafx.beans.property.SimpleStringProperty("");
                    }
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM HH:mm");
                    return new javafx.beans.property.SimpleStringProperty(rdv.getDateHeureDebut().format(formatter));
                }
            });
        ...
        if (colUpcomingPatient != null) {
            colUpcomingPatient.setCellValueFactory(new javafx.util.Callback<javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String>, javafx.beans.value.ObservableValue<String>>() {
                @Override
                public javafx.beans.value.ObservableValue<String> call(javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String> param) {
                    RendezVous rdv = param.getValue();
                    ...
                    return new javafx.beans.property.SimpleStringProperty(rdv.getPatient().getNomComplet());
                }
            });
        ...
        if (colUpcomingDoctor != null) {
            colUpcomingDoctor.setCellValueFactory(new javafx.util.Callback<javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String>, javafx.beans.value.ObservableValue<String>>() {
                @Override
                public javafx.beans.value.ObservableValue<String> call(javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String> param) {
                    RendezVous rdv = param.getValue();
                    ...
                    return new javafx.beans.property.SimpleStringProperty(rdv.getMedecin().getNomComplet());
                }
            });
        ...
        if (colUpcomingMotif != null) {
            colUpcomingMotif.setCellValueFactory(new javafx.util.Callback<javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String>, javafx.beans.value.ObservableValue<String>>() {
                @Override
                public javafx.beans.value.ObservableValue<String> call(javafx.scene.control.TableColumn.CellDataFeatures<RendezVous, String> param) {
                    RendezVous rdv = param.getValue();
                    if (rdv == null) {
                    ...
                    return new javafx.beans.property.SimpleStringProperty(motif != null ? motif : "N/A");
                }
            });
        ...
        System.out.println("=== setupUpcomingAppointmentsTable() COMPLETED ===\n");
    }
    ...
    private void loadUpcomingAppointments() {
        ...
        try {
            ...
            if (!UserSession.isAuthenticated()) {
                ...
                upcomingAppointmentsTable.setItems(FXCollections.observableArrayList());
                if (todayApptCountLabel != null) todayApptCountLabel.setText("(0)");
                return;
            }
            System.out.println("✓ User authenticated");
            
            // Get appointments from service
            List<RendezVous> appointments = dashboardService.getTodayAppointments();
            System.out.println("✓ Retrieved " + appointments.size() + " appointments from service");
            
            // Print each appointment for debugging
            if (appointments.isEmpty()) {
                System.out.println("  WARNING: No appointments found for today!");
            }
            for (int i = 0; i < appointments.size(); i++) {
                RendezVous rdv = appointments.get(i);
                System.out.println("  [" + i + "] ID=" + rdv.getId() + 
                    ", Time=" + rdv.getDateHeureDebut() + 
                    ", Patient=" + (rdv.getPatient() != null ? rdv.getPatient().getNomComplet() : "NULL") +
                    ", Doctor=" + (rdv.getMedecin() != null ? rdv.getMedecin().getNomComplet() : "NULL") +
                    ", Motif=" + rdv.getMotif());
            }
            
            // Force clear and reset
            upcomingAppointmentsTable.getItems().clear();
            System.out.println("✓ Table cleared, current items: " + upcomingAppointmentsTable.getItems().size());
            
            // Set items to table with force refresh
            javafx.collections.ObservableList<RendezVous> items = FXCollections.observableArrayList(appointments);
            upcomingAppointmentsTable.setItems(items);
            System.out.println("✓ Table updated with " + upcomingAppointmentsTable.getItems().size() + " rows");
            
            // Update counter label
            if (todayApptCountLabel != null) {
                todayApptCountLabel.setText("(" + appointments.size() + ")");
                System.out.println("✓ Counter label updated to: (" + appointments.size() + ")");
            }
            ...
            System.err.println("ERROR in loadUpcomingAppointments: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private void initializeCharts() {
        try {
            ...
            Map<String, Integer> weeklyData = dashboardService.getWeeklyAppointments();
    ...
    private void handleQuickPatient() {
        try {
            ...
            if (!rendezVousService.canCreateAppointment()) {
                showAlert("Permission Refusée", "Vous n'avez pas la permission de créer un nouveau patient.", Alert.AlertType.WARNING);
                return;
            }
            ...
            if (mainController != null) {
                ...
                showAlert("Navigation", "Veuillez utiliser la section Patients pour ajouter un nouveau patient.", Alert.AlertType.INFORMATION);
            }
        } catch (Exception e) {
            e.printStackTrace();
            showAlert("Erreur", "Erreur lors de l'accès à la section Patients: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    private void handleQuickAppointment() {
        try {
            // Check if user has permission
            if (!rendezVousService.canCreateAppointment()) {
                showAlert("Permission Refusée", "Vous n'avez pas la permission de planifier un rendez-vous.", Alert.AlertType.WARNING);
                return;
            }
            ...
            if (mainController != null) {
                ...
                showAlert("Navigation", "Veuillez utiliser la section Agenda pour planifier un rendez-vous.", Alert.AlertType.INFORMATION);
            }
        } catch (Exception e) {
            e.printStackTrace();
            showAlert("Erreur", "Erreur lors de l'accès à l'Agenda: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    private void handleQuickInvoice() {
        try {
            // Check if user has permission
            if (!rendezVousService.canCreateAppointment()) {
                showAlert("Permission Refusée", "Vous n'avez pas la permission de créer une facture.", Alert.AlertType.WARNING);
                return;
            }
            ...
            if (mainController != null) {
                ...
                showAlert("Navigation", "Veuillez utiliser la section Facturation pour créer une facture.", Alert.AlertType.INFORMATION);
            }
        } catch (Exception e) {
            e.printStackTrace();
            showAlert("Erreur", "Erreur lors de l'accès à la Facturation: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    private void handleViewReports() {
        try {
            ...
            alert.setContentText("Les rapports suivants seront bientôt disponibles:\n" +
            ...
            e.printStackTrace();
            showAlert("Erreur", "Erreur lors de l'accès aux rapports: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
