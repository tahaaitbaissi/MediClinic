...
public class DoctorController implements Initializable {
    ...
    @FXML private TableView<Medecin> doctorTable;
    @FXML private TableColumn<Medecin, Long> colId;
    @FXML private TableColumn<Medecin, String> colNomComplet;
    ...
    private void setupTableColumns() {
        ...
        colSpecialite.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleStringProperty(
                ...
                cellData.getValue().getSpecialite().name() : "N/A"
            )
        );
    ...
    private void loadDoctors() {
        try {
            doctorList = FXCollections.observableArrayList(medecinService.findAll());
            doctorTable.setItems(doctorList);
        } catch (Exception e) {
            showAlert("Erreur", "Erreur lors du chargement des médecins: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    private void filterDoctors() {
        ...
        // Filter by specialty
        if (selectedSpecialty != null && !selectedSpecialty.equals("Toutes spécialités")) {
            filtered = filtered.stream()
                .filter(med -> med.getSpecialite() != null && 
        ...
        if (searchText != null && !searchText.trim().isEmpty()) {
            ...
            filtered = filtered.stream()
                .filter(med -> 
                    med.getNomComplet().toLowerCase().contains(searchLower) ||
                    (med.getEmail() != null && med.getEmail().toLowerCase().contains(searchLower)) ||
                    (med.getTelephone() != null && med.getTelephone().contains(searchText))
                )
                .collect(Collectors.toCollection(FXCollections::observableArrayList));
        }
    ...
    private void handleAddDoctor() {
        try {
            ...
            specialiteCombo.setItems(FXCollections.observableArrayList(SpecialiteMedecin.values()));
            ...
            specialiteCombo.setOnAction(event -> changeListener.changed(null, "", ""));
    ...
    private void deleteDoctor(Medecin medecin) {
        ...
        alert.setContentText("Êtes-vous sûr de vouloir supprimer " + medecin.getNomComplet() + " ?\n" +
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    loadDoctors();
                    showAlert("Succès", "Médecin supprimé avec succès!", Alert.AlertType.INFORMATION);
                } catch (Exception e) {
    ...
    private void showDoctorDetails(Medecin medecin) {
        ...
        showAlert("Détails Médecin",
                "Nom: " + medecin.getNomComplet() + "\n" +
                "Spécialité: " + (medecin.getSpecialite() != null ? medecin.getSpecialite().name() : "N/A") + "\n" +
                "Email: " + (medecin.getEmail() != null ? medecin.getEmail() : "N/A") + "\n" +
                "Téléphone: " + (medecin.getTelephone() != null ? medecin.getTelephone() : "N/A"),
